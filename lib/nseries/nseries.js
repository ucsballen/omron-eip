...
class NSeries {
  ...
  _snapshotInstance(inst) {
    ...
    if (inst instanceof CIPStructure) {
      ...
      snap.layoutTrusted = inst._layoutTrusted;
      snap.members = {};
  ...
  _instanceFromSnapshot(snap) {
    ...
    if (inst instanceof CIPStructure && snap.members) {
      ...
      // Prefer the stored decision; fall back to recomputing so old snapshots (and any
      // size drift) stay correct rather than silently trusting a bad layout.
      inst._layoutTrusted = (typeof snap.layoutTrusted === 'boolean')
        ? snap.layoutTrusted
        : (inst.computePackedSize() === inst.size);
    } else if (inst instanceof CIPArray) {
      ...
      // fromInstance recomputes size; restore the authoritative size we snapshotted.
      if (typeof snap.size === 'number' && snap.size > 0) inst.size = snap.size;
    }
    return inst;
  }
  ...
  async _structureInstanceFromVariableTypeObject(vto) {
    ...
    // Decide once whether the local byte-packing layout matches the controller's actual
    // layout. It matches for "NJ" offset-type structs and diverges for "User" offset-type
    // structs (hand-placed or bit-packed members). When it diverges we can't slice a single
    // blob correctly — the controller doesn't expose per-member offsets anywhere — so reads
    // and writes fall back to per-member symbolic access. The decision is cached on this
    // instance (and re-derived on dictionary load), so it's computed only once per type.
    struct._layoutTrusted = (struct.computePackedSize() === struct.size);
  ...
  async readVariable(variableName) {
    ...
    if (inst instanceof CIPString || inst instanceof CIPArray ||
      ...
      // Untrusted (User offset-type) structure: the single-blob layout can't be sliced
      // correctly, so read each member by its symbolic path and assemble the object.
      if (inst instanceof CIPStructure && inst._layoutTrusted === false) {
        return this._readStructByMembers(inst, variableName);
      }
  ...
  /**
   * Read a structure member-by-member through symbolic paths (used for "User" offset-type
   * structs whose memory layout the local packer can't reproduce). The controller resolves
   * each member path and does the offset/bit math itself. Recurses naturally: nested structs
   * get their own trusted/untrusted treatment, arrays are read whole (so non-zero bases like
   * [4..15] are handled by the controller).
   */
  async _readStructByMembers(inst, variableName) {
    const out = {};
    for (const [name] of inst.members) {
      out[name] = await this.readVariable(`${variableName}.${name}`);
    }
    return out;
  }
  ...
  async writeVariable(variableName, data) {
    ...
    // Untrusted (User offset-type) structure: write each provided member via its symbolic
    // path. The controller places the bytes; member writes don't need the type CRC and don't
    // clobber siblings. (Not an atomic group write — same as the documented member-write path.)
    if (inst instanceof CIPStructure && inst._layoutTrusted === false) {
      return this._writeStructByMembers(inst, variableName, data);
    }
  ...
  /**
   * Write a structure member-by-member through symbolic paths (used for "User" offset-type
   * structs). Only members present in `data` are written, so partial updates work. Recurses:
   * a nested struct member is written via writeVariable, which re-applies the same decision.
   */
  async _writeStructByMembers(inst, variableName, data) {
    if (data === null || typeof data !== 'object' || Array.isArray(data) || Buffer.isBuffer(data)) {
      throw new TypeError(`writeVariable("${variableName}"): expected a plain object for a structure`);
    }
    for (const [name] of inst.members) {
      if (name in data) await this.writeVariable(`${variableName}.${name}`, data[name]);
    }
  }
